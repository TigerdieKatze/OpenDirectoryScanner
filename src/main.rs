...
impl DirectoryReport {
    ...
    fn print(&self) {
        ...
        println!("Total size: {} bytes ({} MB)", self.total_size, self.total_size / 1_048_576);
    ...
    fn save_to_file(&self, path: &str) -> Result<(), Box<dyn Error>> {
        ...
        writeln!(file, "- Total size: {} bytes ({} MB)", self.total_size, self.total_size / 1_048_576)?;
        ...
        if let Some(file_info) = &self.largest_file {
            ...
            writeln!(file, "- Size: {} bytes ({} MB)", file_info.size, file_info.size / 1_048_576)?;
...
fn scan_directory(url: &str, depth: u32, max_depth: u32, client: &Client, nsfw_detector: &NSFWDetector) -> Result<(DirectoryReport, Vec<FileInfo>), Box<dyn Error>> {
    if depth > max_depth {
    ...
    for element in document.select(&selector) {
        ...
        let size_text = element
            .parent()
            .and_then(|parent| {
                parent.select(&Selector::parse("td:nth-child(2)").unwrap()).next()
            })
                    ...
                    Ok((subdir_report, subdir_files)) => {
                        ...
                        if let Some(subdir_largest_file) = subdir_report.largest_file {
                            if report.largest_file.is_none() || 
                               report.largest_file.as_ref().unwrap().size < subdir_largest_file.size {
